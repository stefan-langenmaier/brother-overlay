...
src_install() {
	...
	sed -n 94,232p "${D}usr/local/Brother/Printer/${MODEL}/cupswrapper/cupswrapper${MODEL}" | sed 's/${printer_model}/dcp9055cdn/g;s/${device_model}/Printers/g;s/${printer_name}/DCP9055CDN/g;s/\\//g' > "${D}usr"/libexec/cups/filter/brother_lpdwrapper_${MODEL} || die
	chmod 0755 "${D}"usr/libexec/cups/filter/brother_lpdwrapper_${MODEL} || die
