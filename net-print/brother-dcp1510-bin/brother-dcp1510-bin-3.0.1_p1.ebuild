...
SRC_URI="http://www.brother.com/pub/bsc/linux/dlf/${BR_PN}lpr-${BR_PV}.i386.rpm
	http://www.brother.com/pub/bsc/linux/dlf/${BR_PN}cupswrapper-GPL_src-${BR_PV}.tar.gz"
...
SRC_LPR_DIR="${S}${DEST}"
SRC_CUPS_DIR="${S}/${BR_PN}cupswrapper-GPL_src-${BR_PV}"
...
src_prepare() {
	...
	cp "${SRC_CUPS_DIR}/PPD/${PPD_FILE}" "${S}${CUPS_PPD_PATH}/${MODEL}.ppd" || die "Preparing CUPS PPD file failed"
	mkdir -p "${S}${CUPS_FILTER_PATH}" || die "Creating CUPS filter wrapper install location failed"
	cp "${SRC_CUPS_DIR}/filter/${CUPS_WRAPPER}" "${S}${CUPS_FILTER_PATH}" || die "Preparing CUPS filter wrapper failed"
